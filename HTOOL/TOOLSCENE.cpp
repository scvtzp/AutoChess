...
void HTOOLMAPSCENE::LoadAni(Game_String _Name)
{
	////유닛
	{
...
void HTOOLMAPSCENE::Load_Functions()
{
	{
		HGAMEDIRECTORY Dic;
...
{
	Game_Ptr<HFBX> TestFbx = HFBX::Find(_FbxObjectData.m_MeshName);
	Game_Ptr<Game_Actor> NewActor = SCENE()->CreateActor();
	NewActor->TRANS()->WSCALE({ 0.1F, 0.1F, 0.1F });

	Game_Ptr<Game_Renderer> NewRenderer = NewActor->CreateCom<Game_Renderer>(0);
	NewRenderer->LROT({ 0.0F, 0.0F, 0.0F });


	Game_Ptr<HFBX> _Fbx = HFBX::Find(_FbxObjectData.m_MeshName);
	HFBXANIDATA* Ani = _Fbx->FindAniData(_FbxObjectData.m_TakeName);

	Game_RightView::View->RangeSetting((int)Ani->TimeStartCount, (int)Ani->TimeEndCount);

	Game_RightView::View->m_PlayBtn.SetWindowTextW(L"정지");
